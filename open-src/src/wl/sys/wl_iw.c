 ...
 *
 * $Id: wl_iw.c,v 1.51.4.9.2.8.6.49.2.35 2010/04/25 12:14:58 Exp $
 */
...
)
{
	...
	case IW_AUTH_CIPHER_GROUP:
		...
		if (paramid == IW_AUTH_CIPHER_PAIRWISE) {
			...
			if (iw->privacy_invoked && !val) {
				WL_WSEC(("%s: %s: Privacy Invoked TRUE but clearing wlsec, "
				"assuming we're a WPS enrolle \n", dev->name, __FUNCTION__));

				if ((error = dev_wlc_intvar_set(dev, "is_WPS_enrollee", TRUE))) {
					WL_WSEC(("Failed to set is_WPS_enrollee iovar \n"));
					return error;
				}
			} else if (val) {
	...
	case IW_AUTH_80211_AUTH_ALG:
		...
		WL_TRACE(("Setting the D11auth %d\n", paramval));
	...
	case IW_AUTH_PRIVACY_INVOKED: {
                ...
                if (paramval == 0) {
                        ...
                        if ((error = dev_wlc_intvar_set(dev, "is_WPS_enrollee", FALSE))) {
                                WL_WSEC(("Falied to clear is_WPS_enrollee iovar\n"));
                                return error;
                        }
                } else {
                        ...
                        if (!IW_WSEC_ENABLED(wsec)) {
                           ...
                           if ((error = dev_wlc_intvar_set(dev, "is_WPS_enrollee", TRUE))) {
                                WL_WSEC(("Falied to set is_WPS_enrollee iovar\n"));
                                return error;
                           }
...
)
{
	...
	if (dwrq->length && extra) {
		...
		if (g_onoff == G_WLAN_SET_OFF) {
#if defined(FIRST_IOCTL_SIMULATES_START)
