 ...
 *
 * $Id: bcmsdh_linux.c,v 1.42.10.11.4.5.6.5 2009/11/12 01:40:48 Exp $
 */
...
struct bcmsdh_hc {
	...
	bool            oob_irq_register; /* Keep track if oob was susscesfully registered */
};
...
int bcmsdh_register_oob_intr(void * dhdp)
{
	...
	sdhcinfo->oob_irq_register = TRUE;
	return 0;
}
...
void bcmsdh_unregister_oob_intr(void)
{
	...
	if (sdhcinfo->oob_irq_register) 	{
		set_irq_wake(sdhcinfo->oob_irq, 0);
		disable_irq(sdhcinfo->oob_irq);	/* just in case.. */
		free_irq(sdhcinfo->oob_irq, NULL);
		sdhcinfo->oob_irq_register = FALSE;
	}
}
