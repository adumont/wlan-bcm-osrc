 ...
 *
 * $Id: dhd_sdio.c,v 1.157.2.27.2.36.4.54.2.27 2010/04/09 03:57:37 Exp $
 */
...
#define DHD_STATIC_DATA_BUFFER
#if defined(DHD_STATIC_DATA_BUFFER)
#include <bcmcdc.h>
#endif
...
/*		WARN_ON(1); */
		DHD_ERROR(("%s: bus down err=0x%X reg=0x%X\n", \
				__FUNCTION__, err, bcmsdh_regfail(sdh)));
		bus->dhd->busstate = DHD_BUS_DOWN;
		dumpreg_sdio(sdh);
	}
}
...
#if defined SDIO_ERR_SIM
	bus->dhd->sdio_err_sim_tick++;
#endif
...
#if defined SDIO_ERR_SIM
if ((bus->dhd->sdio_err_sim_tick  == 200) || (bus->dhd->sdio_err_simulate == TRUE)) {
	printf("\n### SIMULATED ERR sdio_err_sim_tick =%d sdio_err_simulate=%d \n", \
		bus->dhd->sdio_err_sim_tick, bus->dhd->sdio_err_simulate);
	bus->dhd->busstate = DHD_BUS_DOWN;
	bus->intstatus = 0;
	bus->dhd->sdio_err_simulate = FALSE;
	dumpreg_sdio(bus->sdh);
}
#endif /* SDIO_ERR_SIM */
...
dhdsdio_isr(void *arg)
{
	...
	if (bus->dhd->busstate == DHD_BUS_DOWN) {
		if (bus->dhd->sdio_err_reported == FALSE)
...
#if defined(DHD_STATIC_DATA_BUFFER)
int dhdsdio_databuf[(MAX_DATA_BUF + DHD_SDALIGN)/sizeof(int)];
#define ROUND_UP_MARGIN	2048
int dhdsdio_rxbuf[(WLC_IOCTL_MAXLEN + sizeof(cdc_ioctl_t) + ROUND_UP_MARGIN +
	SDPCM_HDRLEN + DHD_SDALIGN + ALIGNMENT-1)/sizeof(int)];
#endif	/* defined(DHD_STATIC_DATA_BUFFER) */
static bool
...
#if !defined(DHD_STATIC_DATA_BUFFER)
		if (!(bus->rxbuf = kmalloc(bus->rxblen, GFP_KERNEL))) {
			DHD_ERROR(("%s: MALLOC of %d-byte rxbuf failed\n",
			           __FUNCTION__, bus->rxblen));
			goto fail;
		}
#else	/* !defined(DHD_STATIC_DATA_BUFFER) */
		bus->rxbuf = (uint8 *)dhdsdio_rxbuf;
#endif /* !defined(DHD_STATIC_DATA_BUFFER) */
...
#if !defined(DHD_STATIC_DATA_BUFFER)
	if (!(bus->databuf = kmalloc(MAX_DATA_BUF, GFP_KERNEL))) {
...
#else /* !defined(DHD_STATIC_DATA_BUFFER) */
	bus->databuf = (uint8 *)dhdsdio_databuf;
#endif /* !defined(DHD_STATIC_DATA_BUFFER) */
...
dhdsdio_probe_init(dhd_bus_t *bus, osl_t *osh, void *sdh)
{
	...
	bus->dhd->busstate = DHD_BUS_DOWN;
	bus->dhd->sdio_err_reported = FALSE;
#if defined SDIO_ERR_SIM
	bus->dhd->sdio_err_sim_tick = 0;
#endif /* SDIO_ERR_SIM */
	bus->sleeping = FALSE;
	bus->rxflow = FALSE;
...
dumpreg_sdio(void *sdh)
{
	...
	/*  F0 dump  */
	printf("==== Registers F0 dump===\n");
	for (i = 0; i <= 13; i++) {
		printf("F0[0x%X]=0x%x\n",  i,  bcmsdh_cfg_read(sdh, SDIO_FUNC_0, i, NULL));
	}
	/* F1 dump */
	printf("==== Registers F1 dump===\n ");
	for (i = 0; i <= 13; i++) {
		reg = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_SPROM_CS + i, &err);
		if (!err)  printf("F1[0x%X]=0x%x\n", SBSDIO_SPROM_CS + i, reg);
		else printf("Failed to read F1[0x%X]\n", SBSDIO_SPROM_CS + i);
	}
}
