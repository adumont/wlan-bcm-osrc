 ...
 *
 * $Id: dhd_common.c,v 1.5.6.8.2.8.2.25.2.1 2010/03/24 06:44:53 Exp $
 */
#include <typedefs.h>
#include <osl.h>
...
#if defined SDIO_ERR_SIM
	{"sdio_err_sim", IOV_SDIO_ERROR_SIM, 0, IOVT_VOID,	0 },
#endif
...
#if defined SDIO_ERR_SIM			
printf("Status: current dhd_pub->up=%d dhd_pub->pub.busstate=%d\n", \
	dhd_pub->up, dhd_pub->busstate);
dhd_pub->up = 0;
printf("Simulate if_down:  dhd_pub->up=%d\n", dhd_pub->up);
#endif
		break;
...
#if defined SDIO_ERR_SIM
	case IOV_SVAL(IOV_SDIO_ERROR_SIM):
		if (dhd_pub->sdio_err_simulate == FALSE) {
			printf("Forced SDIO simulated error for debugging\n");
			dhd_pub->sdio_err_simulate = TRUE;
		}else {
			printf("Clean up SDIO simulated error for debugging\n");
			dhd_pub->sdio_err_simulate = FALSE;
		}
	break;
#endif
