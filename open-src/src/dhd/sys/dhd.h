 ...
 *
 * $Id: dhd.h,v 1.32.4.7.2.4.28.28.2.10 2010/03/24 06:36:18 Exp $
 */
...
typedef struct dhd_pub {
	...
	bool sdio_err_reported;   /* SDIO BUS ERROR  reported */
#if defined SDIO_ERR_SIM
	bool sdio_err_simulate;   /* SDIO BUS ERROR  simulataion */
	uint  sdio_err_sim_tick;  /* SDIO Bus Error simulation tick counter */
#endif
...
#if (defined(LINUX) || 1) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && \
...
extern int dhd_add_if(struct dhd_info *dhd, uint8 ifidx, void *handle, char *name, uint8 *mac);
