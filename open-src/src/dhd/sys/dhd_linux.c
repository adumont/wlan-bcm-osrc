 ...
 *
 * $Id: dhd_linux.c,v 1.65.4.9.2.13.6.41.2.19 2010/03/24 17:08:45 Exp $
 */
...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	/* Reject if ifconfig down was already called */
	if (!dhd->pub.up) {
		DHD_ERROR(("%s: xmit rejected due to ifconfig_down \n", __FUNCTION__));
		netif_stop_queue(net);

		/* Incorrect if_down behavier. ifconfig down was called but TX packet is comming */
		dhd->pub.up = 1;
		netif_start_queue(net);
		return -ENODEV;
	}

	/* Reject if SDIO Bus down */
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (!dhd->pub.up || (dhd->pub.busstate != DHD_BUS_DATA)) {
		...
		/* Greg if bus down problem reported send Private Event BUSERROR to upper layer */
		if ((dhd->pub.busstate == DHD_BUS_DOWN) && (dhd->pub.sdio_err_reported == FALSE)) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && 1
			union iwreq_data wrqu;
			char extra[IW_CUSTOM_MAX + 1];

			WAKE_LOCK_INIT(&dhd->pub, WAKE_LOCK_IOCTL, "SDIO ERROR REPORT");
			WAKE_LOCK(&dhd->pub, WAKE_LOCK_IOCTL);
#endif 

			DHD_ERROR(("DONGLE_DOWN : dhd->pub.up=%d dhd->pub.busstate=%d\n", \
				dhd->pub.up, dhd->pub.busstate));

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && 1
			memset(&wrqu, 0, sizeof(wrqu));
			/* sending Event named SDIOERROR up */
			strcpy(extra, "SDIOERROR");
			wrqu.data.length = strlen(extra);
			wireless_send_event(net, IWEVCUSTOM, &wrqu, extra);
			DHD_ERROR(("###>> %s Send IWEVCUSTOM Event as %s\n", __FUNCTION__, extra));
			/* Clean up */
			dhd->pub.sdio_err_reported = TRUE;
...
int
dhd_add_if(dhd_info_t *dhd, uint8 ifidx, void *handle, char *name, uint8 *mac_addr)
{
	dhd_if_t *ifp;
...
dhd_net_attach(dhd_pub_t *dhdp, int ifidx)
{
	...
	/* forced broadcast scan just after netdev registration */
